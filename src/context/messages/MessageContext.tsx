...
type MessageAction =
  ...
  | {
      ...
      payload: { messageId: number; status: "sent" | "delivered" | "read" };
    }
  | { type: "SET_LOADING_CONVERSATIONS"; payload: boolean }
  | {
      type: "SET_LOADING_MESSAGES";
      payload: { userId: number; loading: boolean };
    }
  ...
  useEffect(() => {
    ...
    MessageService.onNewMessage((message) => {
      dispatch({
        ...
        payload: {
          userId:
            message.sender_id === state.activeConversation?.user_id
              ? message.sender_id
  ...
  const loadMessages = useCallback(async (userId: number) => {
    ...
    try {
      ...
      if (response.success && response.data?.messages) {
        dispatch({
          type: "SET_MESSAGES",
          payload: { userId, messages: response.data.messages },
        });
      }
    } catch (error) {
      const errorMessage =
