...
export const useTextSelection = (containerRef: React.RefObject<HTMLElement | null>): UseTextSelectionReturn => {
  const [toolbarVisible, setToolbarVisible] = useState(false);
  const [toolbarPosition, setToolbarPosition] = useState({ top: 0, left: 0 });
  const [selectedText, setSelectedText] = useState('');
  const [selectionData, setSelectionData] = useState<SelectionData>({ text: '', range: null });
